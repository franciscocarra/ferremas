...
def producto(request):
    ...
    # 1. Sacamos las marcas del JSON para construir el menú lateral
    marcas_existentes = []
...
# views.py - Segmento actualizado de iniciar_session
@csrf_exempt
def iniciar_session(request):
    if request.method == 'POST':
        ...
        # Autenticamos usando tu PersonaAuthBackend
        user = authenticate(request, username=email, password=password)

        if user is not None:
            # ¡ESTO ES LO MÁS IMPORTANTE! Crea la cookie de sesión de Django
            login(request, user)
            
            # Buscamos el tipo de persona para decirle al frontend a dónde redirigir
            try:
...
def confirmar_pago(request):
    ...
    try:
        ...
        if isinstance(response, dict) and response.get('status') == 'AUTHORIZED' and response.get('response_code') == 0:
            ...
            # --- INICIO AGREGADO: Diccionario de errores ---
            codigos_error = {
                -1: "Rechazo de transacción (posible falta de fondos o tarjeta inválida).",
                -2: "La transacción debe reintentarse.",
                -3: "Error en la transacción con el banco.",
                -4: "Rechazo de transacción (tarjeta denegada).",
                -5: "Rechazo por error de tasa.",
                -6: "Excede el cupo máximo mensual.",
                -7: "Excede el límite diario por transacción.",
                -8: "Rubro no autorizado.",
                1: "El pago fue cancelado o rechazado por el banco emisor."
            }
            
            # Obtenemos el código y generamos el mensaje amigable
            codigo_bd = response.get('response_code') if isinstance(response, dict) else None
            mensaje_amigable = codigos_error.get(codigo_bd, "Rechazo desconocido por parte del sistema.")
            # --- FIN AGREGADO ---

            context = {
                'response': response,
                'mensaje_razon': mensaje_amigable # Añadimos el mensaje al contexto para el HTML
            }
        ...
        # --- INICIO AGREGADO: Retorno a pago_fallido.html con mensaje amigable ---
        return render(request, 'core/pago_fallido.html', {
            'mensaje_razon': "Pago anulado o error de conexión con Transbank."
        })
...
def administracion(request):
    ...
    if request.user.is_superuser:
        ...
        try:
            ...
            # Normalizar el tipo de persona para la comparación
            tipo = persona.tipo_persona.strip().lower()
    ...
    if not tiene_permiso:
        # Si no tiene permiso, lo enviamos al home con un mensaje
...
def inventario(request):
    try:
        ...
        # Usamos .get('sku') para evitar que se caiga si un producto viene sin SKU
        skus_en_api = {prod_api.get('sku') for prod_api in productos_api if prod_api.get('sku')}
        ...
        for prod_api in productos_api:
            # ESTE PRINT TE SALVARÁ LA VIDA: Mira tu terminal para ver cómo se llama realmente el campo
            print("DATO DE LA API:", prod_api)
            
            # Verificamos que al menos traiga un SKU válido antes de intentar guardar
            if prod_api.get('sku'):
                Producto.objects.update_or_create(
                    sku=prod_api.get('sku'),
                    defaults={
                        # Usamos .get() en todos lados. Si la llave no existe, pone el valor de la derecha.
                        # Intentará buscar 'nombre_producto', si no está, intentará 'nombre', y si no, pondrá 'Sin Nombre'
                        'nombre_producto': prod_api.get('nombre_producto', prod_api.get('nombre', 'Sin Nombre')),
                        'marca': prod_api.get('marca', 'Sin Marca'),
                        'descripcion': prod_api.get('descripcion', ''),
                        'precio': prod_api.get('precio', 0),
                        'stock': prod_api.get('stock', 0),
        ...
        # Esto atrapará cualquier otro error raro para que la página no se caiga
...
def lista_usuarios(request):
    # Cambiamos __iexact por __icontains. 
    # Así excluirá "admin", "Admin", "Administrador", "administrador", etc.
    usuarios = Persona.objects.exclude(tipo_persona__icontains='admin') 
...
@csrf_exempt # Usamos esto temporalmente si el JS no envía el token CSRF correctamente
def enviar_contacto(request):
    if request.method == 'POST':
        try:
            ...
            return JsonResponse({'mensaje': 'Tu mensaje ha sido enviado al administrador correctamente.'}, status=200)
        except Exception as e:
            return JsonResponse({'error': f'Error al procesar el mensaje: {str(e)}'}, status=400)
    
    return JsonResponse({'error': 'Método no permitido'}, status=405)
# Asegúrate de haber importado el modelo MensajeContacto que creamos en los pasos anteriores
from .models import MensajeContacto 

def lista_mensajes(request):
    # Obtenemos todos los mensajes ordenados del más nuevo al más viejo
